...
def thread(group, thread):
    ...
    return render_template("thread_page.html", username=username, img_src=profile_pic, **rights, groupname=group.Name.replace("_", " "), threadname=thread.Name, description=thread.Description, posts)
    return render_template("group_page.html",  private=private, closed=closed, threads=threads, form=form)
